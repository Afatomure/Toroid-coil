...
void setup() {
  ...
  // ToDo: надо подключить другой дисплей (не МЭЛТ) + модуль I2C, а на какой-то из освободившихся пинов подключить этот драйвер (PWM_PIN)
  ...
  switch (screen_mode) {
    ...
    case 3:
      while (current_turns != turns) {
         ...
         * Запустить мотор диска. После 1-ой обмотки(когда сработает датчик линии) сделать один шаг тороида на угол определяющий плотность намотки.
         * Ждать, когда закончится 2-ая обмотка.
         * Сделать еще один шаг, и так, пока кол-во оборотов (кол-во срабатываний датчика линии) не будет соответствовать заданному на первой странице кол-ву.
         * (Мотор диска при этом не останавливается)-^
         * Остановить все и переключиться на 4-ый экран.
         */
